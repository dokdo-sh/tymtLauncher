...
export const Chat = (props: any) => {
  ...
  const handleSaveHistory = async (msg: MSG) => {
    ...
    const filtered = history.filter(
      (item) => item.createdAt > curTime - SAVE_DURATION
    );
  ...
  useEffect(() => {
    ...
    if (newMsg) {
      ...
      setMsgReceived((state) => [newMsg, ...state]);
      ...
      <div className="flex flex-row h-screen">
        ...
        <div className="grow flex flex-col">
          {partnerName.length > 0 && (
            <div className="py-2 px-4 border-b border-gray-800 flex flex-row items-center top-0 right-0 left-0">
              <div className="grow">
                <div className="font-bold py-1">
                  {partnerName.length == 0
                    ? "[User]"
                    : partnerName.substring(0, 4) +
                      "-" +
                      partnerName.substring(partnerName.length - 4)}
                </div>
          ...
          <Messages myId={myId} partnerName={partnerName} msglist={roomMsg} />
          <SendMessage
            myId={myId}
            to={partnerName}
