            ...
            <div className="max-w-7xl mx-auto pb-16 px-8">
                <div className="py-16">
                    <div className="flex flex-row max-w-3xl space-x-3 mt-8">
                        ...
                        <Button className="bg-[#f64a28] hover:bg-greenish" onClick={() => navigate("/wallet/staking")}> 
                            <img src="/blockchains/solar.png" className="w-6 mr-2" alt="" /> Staking
                        </Button>
                ...
                <div className="grid grid-cols-6 space-x-2">
                    <div className="col-span-4 border border-gray-800 w-full rounded divide-y divide-gray-800 cursor-pointer">
                        {Object.keys(TymtCore.Blockchains).map((key: string) => { 
                            return { key: key as BlockchainKey, data: TymtCore.Blockchains[key as BlockchainKey] } })
                            .map((blockchain) => (
                            <div className={`py-4 px-4 hover:bg-gray-800/50 ${selectedBlockchain == blockchain.key ? 'bg-gray-800/50' : ''}`} 
                                onClick={() => { setSelectedBlockchain(blockchain.key) }} key={blockchain.key}
                            >
                    ...
                    <div className="col-span-2">
                        <div className="bg-gray-800/50 py-8 rounded border border-gray-800" style={{background: 'white'}}>
                            <div className="pb-6 text-center text-2xl ">Deposit</div>
                            <QRCode value={addresses[selectedBlockchain] ? addresses[selectedBlockchain].address : ""} className="mx-auto" />
                            <div className="py-4">
                                <div className="grow w-fit rounded-lg text-center select-none cursor-pointer text-sm rounded-full bg-gray-500/100 hover:bg-gray-500 hover:text-black px-2 py-1 mx-auto text-primary hover:text-white">
                                    <div className="font-mono" 
                                        onClick={() => { 
                                            writeText(addresses[selectedBlockchain] ? addresses[selectedBlockchain].address : "").then(() => { toast.success("Copied!", { autoClose: 2000 }) }) }}
                                        >
                                        <BiCopy className="text-black inline-block" />  &nbsp;
                                        <span className="text-xs">{addresses[selectedBlockchain] ? addresses[selectedBlockchain].address : ""}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
