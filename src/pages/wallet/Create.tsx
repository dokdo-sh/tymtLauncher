  ...
  return (
    <div className="mx-auto max-w-3xl py-safe">
      <div className="py-4">
        ...
        >
          {" "}
          <MdArrowBack className="text-2xl" /> Go back to login
        </a>
      </div>
      <Wizard>
    ...
    <div className="px-6">
      <div className="">
        ...
        <div className="text-gray-400 text-center  pt-8">
          <p>
            Remember, saving your mnemonic in a safe place is the only get to
            recover your wallet!
          </p>
          <br />
          <p>Click on refresh to generate a new mnemonic</p>
        </div>
        <div className="py-5"></div>
        <div className="flex flex-row mb-2 w-full">
          <div className="grow"></div>
          <Button
            className="w-fit bg-red-500/50 hover:bg-red-500 px-2"
            onClick={() => {
              refreshMnemonic();
            }}
          >
            <BiRefresh />
            <span className="text-xs pl-2">Refresh</span>
          </Button>
        </div>
        <div className="rounded-t bg-dark-tertiary mx-auto  py-4 text-sm text-center select-none hover:bg-dark-hoverish font-mono  font-bold text-base  ">
          <div className="flex flex-row w-fit mx-auto gap-3 px-3 flex-wrap items-center justify-items-center">
            {mnemonic.split(" ").map((word) => (
              <div className="border border-gray-800 text-gray-400 px-1 py-1 rounded">
                {word}
              </div>
            ))}
          </div>
    ...
    <div className="px-6">
      <div className="">
        ...
        <div className="flex flex-row mt-3">
          <div className="grow"></div>
          <Button
            className="w-fit bg-red-500/50 hover:bg-red-500 px-2 border border-dark-secondary flex items-center ease-in duration-300 text-sm"
            onClick={() => {
              copyPP();
            }}
          >
        ...
        <div className="grid grid-cols-3 gap-3 mx-auto mb-8 mt-6">
          {props.passphrase.split(" ").map((word, i) => (
            <div className="px-2  py-3 rounded-lg items-center flex bg-gray-800">
              {" "}
              <span className="text-xs text-gray-400 mx-2 mr-2 select-none">
                {i + 1}
              </span>{" "}
              <span className="text-sm font-black mx-auto">{word}</span>
            </div>
          ))}
        </div>
      </div>
...
const VerifyWallet = (props: { passphrase: string }) => {
  ...
  function isVerified(): boolean {
    let splitted = props.passphrase.split(" ");
    return (
      input1 == splitted[2] && input2 == splitted[5] && input3 == splitted[8]
    );
  }
  ...
  return (
    <div className="px-6">
      ...
      <div className="py-5 flex w-fit mx-auto">
        <div className="px-3">
          <div className="text-center py-4 font-black text-sm">Third word</div>
          <input
            type="text"
            name=""
            className={`text-center p-3  bg-dark-secondary rounded-lg outline-greenish w-40 ${
              input1 != correct1 ? "bg-dark-secondary" : "bg-green-800"
            }`}
            value={input1}
            onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
              if (input1 != correct1) {
                setInput1(e.currentTarget.value);
                if (correct1 == e.currentTarget.value) {
                  i2.current.focus();
                }
              }
            }}
            ...
            readOnly={input1 == correct1}
            ref={i1}
          />
        </div>
        <div className="px-3">
          <div className="text-center py-4 font-black text-sm">Sixth word</div>
          <input
            type="text"
            name=""
            ref={i2}
            className={`text-center p-3  bg-dark-secondary rounded-lg outline-greenish w-40 ${
              input2 != correct2 ? "bg-dark-secondary" : "bg-green-800"
            }`}
            value={input2}
            onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
              if (input2 != correct2) {
                setInput2(e.currentTarget.value);
                if (correct2 == e.currentTarget.value) {
                  i3.current.focus();
                }
              }
            }}
            ...
            readOnly={input2 == correct2}
          />
        </div>
        <div className="px-3">
          <div className="text-center py-4 font-black text-sm">Ninth word</div>
          <input
            type="text"
            ref={i3}
            value={input3}
            ...
            name=""
            className={`text-center p-3  w-40 bg-dark-secondary rounded-lg outline-greenish ${
              input3 != correct3 ? "bg-dark-secondary" : "bg-green-800"
            }`}
            readOnly={input3 == correct3}
            onFocus={() => {
              refreshShuffle();
              setCurrentInput("3");
            }}
      ...
      <div className="grid grid-cols-3 gap-3 mx-auto mb-8 mt-6">
        {shuffledPassphrase.map((word, i) => (
          <div
            className="px-2 border py-2 border-dark-secondary rounded-lg items-center flex hover:bg-dark-secondary cursor-pointer"
...
const WalletSetup = (props: { passphrase: string; password: string }) => {
  ...
  async function getAddresses() {
    Object.keys(TymtCore.Blockchains)
      .map((key: string) => {
        return {
          key: key as BlockchainKey,
          data: TymtCore.Blockchains[key as BlockchainKey]
        };
      })
  ...
  return (
    <div className="px-4">
      <div className="pb-10">
        ...
        <div className="rounded-t bg-dark-tertiary mx-auto  py-4 text-sm text-center select-none hover:bg-dark-hoverish font-mono  font-bold text-base  ">
          <div className="flex flex-wrap flex-row w-fit mx-auto gap-3 px-3">
            {props.passphrase.split(" ").map((word) => (
              <div className="border border-gray-800 text-gray-400 px-1 py-1 rounded">
                {word}
              </div>
            ))}
            ...
            .map((blockchain) => (
              <div className="py-4 px-4 hover:bg-gray-800">
                <div className="flex flex-row items-center space-x-4">
                  <img
                    src={`/blockchains/${blockchain.key}.png`}
                    className="w-12"
                    alt=""
                  />
                  <div className="grow">
                    <div className="font-bold text-xl">
                      {blockchain.data.name}
                    </div>
      ...
      <div className="flex pt-2 flex-row-reverse items-center justify-items-center pb-10">
        <Button
          onClick={() => {
            TymtCore.Launcher.Settings.save(
              { mnemonic: props.passphrase, addresses: {} },
              props.password
        ...
        <a
          href="#"
          className="hover:underline mx-3 pb-4"
          onClick={() => {
            previousStep();
          }}
        >
