            ...
            <div className="min-w-1/2 max-w-4xl mx-auto px-8">
                <div className="border border-primary p-1 rounded space-y-3 bg-black/40">
                    <div className="border-2 border-red-500 rounded space-y-4 p-6 w-full h-full">
                        ...
                        <div className="">
                            <div className=" text-center py-4 text-xs">Enter your password to decrypt your mnemonic.</div>
                            <div className="mx-auto w-fit my-2">
                                <input type="password" value={password} onKeyDown={(e) => {if (e.key === 'Enter') {login()}}} onChange={(e) => setPassword(e.currentTarget.value)} className="rounded bg-white/10 py-1 px-2" placeholder="Password"/>
                            </div>
                            { wrongPassword && <div className="text-red-800 text-center text-xs pb-4">Password is not correct</div> }
                            <div className="py-3 px-9 mx-auto">
                                <Button className="w-full text-center font-bold" onClick={login}>
        ...
        <div className="max-w-2xl mx-auto my-16 px-4">
            <div className="border border-primary p-1 rounded space-y-3">
                <div className="border border-red-500 rounded space-y-4 p-6 w-full h-full">
                    ...
                    <div className='bg-primary/70 py-2 mx-auto px-6 text-base w-fit  hover:bg-primary cursor-pointer hover:ease-in duration-200 flex flex-row items-center h-fit' onClick={() => proceed("import")}> 
                        <div>Import mnemonic</div>
                    </div>
                    <div className='bg-red-500/70 py-2 mx-auto px-6 text-base w-fit  hover:bg-red-500 cursor-pointer hover:ease-in duration-200 flex flex-row items-center h-fit' onClick={() => proceed("create")}> 
                        <div>Create mnemonic</div>
                    </div>
                    <div className="text-center text-xs">tymt.com</div>
                </div>
            </div>
        </div>
    );
