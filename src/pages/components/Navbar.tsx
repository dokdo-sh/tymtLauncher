    ...
    return (
        <div className=" border-b border-gray-800 top-0 sticky bg-black bg-opacity-90 z-50 shadow-lg shadow-black">
                <div className="flex flex-row items-center justify-items-center max-w-7xl mx-auto">
                        <div className="">
                        <div className="mx-4 flex flex-row items-center my-1 w-[44px] rounded-lg border-2 border-transparent hover:border-white p-2 bg-red-500 cursor-pointer opacity-80 hover:opacity-100 ease-in duration-200" onClick={() => {dispatch(change(undefined));navigate("/")}  }>
                             ...
                             {!game &&
                                <div className="space-x-3">
                                    <div className='flex flex-row h-full space-x-3'>
                                        <input type="text" className='rounded bg-gray-800 bg-opacity-50 py-1 px-3 w-[300px]' placeholder={window.location.pathname == "/library"? 'Search libraries': 'Search games'} onChange={(e) =>{ setSearchText(e.currentTarget.value); dispatch(changeSearch(e.currentTarget.value))}} value={searchText}/>
                                    <div>
                        ...
                        <div className='pl-5 pr-5 text-s hover:text-red-500 hover:cursor-pointer rounded' onClick={() => { dispatch(changeSession({logged:false, password:undefined}));navigate("/session")}}> 
                            <RiLogoutCircleLine/> 
                        </div>
                </div>                
        </div>
