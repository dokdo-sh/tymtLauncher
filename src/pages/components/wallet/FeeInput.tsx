...
export const FeeInput = (props: {onValidFee: (fee:number) => void, wallet: IWallet}) => {
    ...
    return (
        <div className="py-3">
            ...
            <div className="px-3">
            {edit && <>{props.wallet.ticker} <input type="text" className={`w-28 text-center rounded text-sm  focus:outline-none ml-1 p-1 ${!edit? 'bg-dark-primary': 'bg-dark-secondary'}`} value={fee} onChange={(e:any) => {updateFee(e.currentTarget.value)}}/>
            </>}
            {!edit && <div className="flex flex-row"> <div> {props.wallet.ticker}</div>
            ...
            {!edit && <div className="text-xs hover:underline text-greenish cursor-pointer" onClick={() => setEdit(true)}>{t("edit_fee")}</div>}
            </div>
