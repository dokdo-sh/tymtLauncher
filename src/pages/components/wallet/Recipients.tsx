...
export const Recipients = (props: {onChange?: (recipients:Array<{address:string, amount:string}>) => void}) => {
    ...
    useEffect(() => {
        const chain = TymtCore.Blockchains[wallet.addresses.key as BlockchainKey]
        chain.wallet.getBalance(wallet.addresses.address).then((bal) => {
    ...
    return (
        <div>
            <div className="py-2 bg-black px-2 rounded-lg">
                ...
                <span className="pl-2 text-gray-400 text-xs">({(currentBalance)} {t("available")})</span>
                <div className="pt-4">
                    <Button
                        className={`w-fit py-1 px-2`}
                        disabled={!addEnabled || !search}
                        onClick={() => {
                        if (addEnabled) {
                            addRecipient();
                            changeSearch("");
                    ...
                    {recipients.map((recipient) => (
                        <div className="text-xs py-2 rounded border hover:bg-dark-tertiary select-none border-dark-secondary my-1 px-2 flex">
                            <div className="grow">
                                ...
                                <div>{t("amount")}: {recipient.amount} {TymtCore.Blockchains[wallet.addresses.key as BlockchainKey].ticker}</div>
                            </div>
